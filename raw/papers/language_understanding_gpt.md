...
$$L_1(\mathcal{U}) = \sum_i \log P(u_i | u_{i-k}, \dots, u_{i-1}; \Theta) \eqno(1)$$
...
$$h_0 = UW_e + W_p$$
$$h_l = \text{transformer\_block}(h_{l-1}) \forall i \in [1, n] \eqno(2)$$
...
$$P(y | x^1, \dots, x^m) = \text{softmax}(h_l^m W_y). \eqno(3)$$

This gives us the following objective to maximize:

$$L_2(\mathcal{C}) = \sum_{(x,y)} \log P(y | x^1, \dots, x^m). \eqno(4)$$

We additionally found that including language modeling as an auxiliary objective to the fine-tuning helped learning by (a) improving generalization of the supervised model, and (b) accelerating convergence. This is in line with prior work [50, 43], who also observed improved performance with such an auxiliary objective. Specifically, we optimize the following objective (with weight $\lambda$):

$$L_3(\mathcal{C}) = L_2(\mathcal{C}) + \lambda * L_1(\mathcal{C}) \eqno(5)$$
...
graph TD
    subgraph Transformer_Architecture [Transformer Architecture]
        direction TB
        TP[Text Prediction] --- TC[Task Classifier]
        subgraph 12x_Block [12x]
            LN1[Layer Norm]
            FF[Feed Forward]
            LN2[Layer Norm]
            MSA[Masked Multi Self Attention]
            LN1 --- FF
            FF --- LN2
            LN2 --- MSA
        end
        TPE[Text & Position Embed] --- 12x_Block
    end

    subgraph Input_Transformations [Input Transformations]
        direction TB
        subgraph Classification
            C1[Start] --> C2[Text] --> C3[Extract] --> C4[Transformer] --> C5[Linear]
        end
        subgraph Entailment
            E1[Start] --> E2[Premise] --> E3[Delim] --> E4[Hypothesis] --> E5[Extract] --> E6[Transformer] --> E7[Linear]
        end
        subgraph Similarity
            S1[Start] --> S2[Text 1] --> S3[Delim] --> S4[Text 2] --> S5[Extract] --> S6[Transformer]
            S7[Start] --> S8[Text 2] --> S9[Delim] --> S10[Text 1] --> S11[Extract] --> S12[Transformer]
            S6 & S12 --> S13((+)) --> S14[Linear]
        end
        subgraph Multiple_Choice
            MC1[Start] --> MC2[Context] --> MC3[Delim] --> MC4[Answer 1] --> MC5[Extract] --> MC6[Transformer] --> MC7[Linear]
            MC8[Start] --> MC9[Context] --> MC10[Delim] --> MC11[Answer 2] --> MC12[Extract] --> MC13[Transformer] --> MC14[Linear]
            MC15[Start] --> MC16[Context] --> MC17[Delim] --> MC18[Answer N] --> MC19[Extract] --> MC20[Transformer] --> MC21[Linear]
        end
    end
```
...
<sup>2</sup>[https://ftfy.readthedocs.io/en/latest/](https://ftfy.readthedocs.io/en/latest/)

<sup>3</sup>[https://spacy.io/](https://spacy.io/)
...
<table>
  ...
  <tbody>
    ...
    <tr>
        <td>Stochastic Answer Network [35] (3x)</td>
        <td><u>80.6</u></td>
        <td><u>80.1</u></td>
        <td>-</td>
        <td>-</td>
    ...
    <tr>
        <td>Multi-task BiLSTM + Attn [64]</td>
        <td>72.2</td>
        <td>72.1</td>
        <td>-</td>
        <td>-</td>
    ...
    <tr>
        <td>Finetuned Transformer LM (ours)</td>
...
<table>
  ...
  <tbody>
    ...
    <tr>
        <td>Hidden Coherence Model [7]</td>
        <td><u>77.6</u></td>
        <td>-</td>
        <td>-</td>
        <td>-</td>
    ...
    <tr>
        <td>Dynamic Fusion Net [67] (9x)</td>
        <td>-</td>
        <td>55.6</td>
        <td>49.4</td>
        <td>51.2</td>
...
<table>
  ...
  <tbody>
    ...
    <tr>
        <td>Single-task BiLSTM + ELMo + Attn [64]</td>
        <td>35.0</td>
        <td>90.2</td>
        <td>80.2</td>
        <td>55.5</td>
    ...
    <tr>
        <td>Multi-task BiLSTM + ELMo + Attn [64]</td>
        <td>18.9</td>
...
<table>
  <thead>
    <tr>
        <th># of layers transferred</th>
        <th>RACE Dev</th>
        <th>RACE Train</th>
        <th>MultiNLI Dev</th>
        <th>MultiNLI Train</th>
    </tr>
  </thead>
  <tbody>
    <tr>
...
<table>
  <thead>
    <tr>
        <th># of pre-training updates</th>
        <th>sentiment analysis</th>
        <th>winograd schema resolution</th>
        <th>linguistic acceptability</th>
